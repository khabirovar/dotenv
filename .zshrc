...
getbranch() {
    ...
    [[ ! -z "$br" ]] && echo '-(%B%F{red}'"git:${br}"'%b%F{%(#.blue.green)})' || echo -n ""
}

getkube() { 
    conf=$(echo $KUBECONFIG | awk -F '/' '{ print $5 }') 
    ns=$(kubectl config get-contexts | grep '*' | awk '{ print $5 }' 2>/dev/null)
    [[ ! -z $conf ]] && echo '-[%B%F{green}'"k8s:${conf}-ns:${ns}"'%b%F{%(#.blue.green)}]' || echo -n ""
}

# -[%B%F{green}$(getkubeinfo)%b%F{%(#.blue.green)}]
...
configure_prompt() {
    ...
    case "$PROMPT_ALTERNATIVE" in
        twoline)
            PROMPT=$'%F{%(#.blue.green)}┌──${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B%F{%(#.red.blue)}%n'$prompt_symbol$'%m%b%F{%(#.blue.green)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}]$(getbranch)$(getkube)\n└─%B%(#.%F{red}#.%F{blue}$)%b%F{reset} '
...
function decode() { echo -n "$1" | base64 -d }
